...
ENV TS_VERSION="3.0.13.4" TS_SERVER_PLATFORM="linux_amd64" TS_USER="teamspeak" TS_HOME="/teamspeak" TS_INJECTS="/opt/teamspeak_injects" LANG="en_US.UTF-8"

RUN apt-get update && \
    apt-get -y upgrade && \
    apt-get clean && \
    apt-get install -y bzip2 wget libmariadb2 netcat supervisor && \
